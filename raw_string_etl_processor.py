...
for daily_transaction in daily_transactions_split:
   list_2 = []
   for data_point in daily_transaction:
    list_2.append(data_point.strip(' '))
   transactions_clean.append(list_2)
...
total_sales = 0
for sale in sales:
  sale_stripped = sale.replace("$", "").strip()
  floated = float(sale_stripped)
  total_sales += floated
print(total_sales)
print("------------------------\n")
...
for thread_sold in thread_sold_split:
  print("{} threads of {} thread were sold today".format(color_count(thread_sold), thread_sold))
